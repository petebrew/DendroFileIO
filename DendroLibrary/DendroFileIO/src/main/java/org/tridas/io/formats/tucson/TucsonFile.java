 ...
 * </table>
 * <h3>Processed Samples</h3> //TODO WRITE ME <h3>Reference</h3>
 * <p>
...
public class TucsonFile implements IDendroFile {
	...
	/**
	 * Set the lat long for this data. Latitude and longitude should be decimal degrees
	 * and in
	...
	public void setLatLong(Double latitude, Double longitude) {
		...
		if (latitude == null || longitude == null) {
			...
			defaults.addConversionWarning(new ConversionWarning(WarningType.NULL_VALUE, I18n
					.getText("tucson.latlong.notnull"), I18n.getText("tucson") + "." + I18n.getText("tucson.latlong")));
		}
		...
		defaults.getStringDefaultValue(TucsonField.LATLONG).setValue(
				ddToDDMMString(latitude, LatLong.LATITUDE) + ddToDDMMString(longitude, LatLong.LONGITUDE));
	}
	 ...
	 * effect of making years after 2000AD invalid as Tucson only allows four chars for
	 * each
	 * year representation.
	...
	public void setUsePlus8000WorkAround(Boolean b) {
		useEightThousandYearOffsetBodge = b;
	}
		...
		for (ITridasSeries series : seriesList) {
			...
			for (;;) {
				...
				if (y.compareTo(end) >= 0 || (isProcessed && y.compareTo(start) < 0)) {
					...
					string.append(StringUtils.leftPad(data.get(y.diff(start)).getValue().toString(), (isProcessed
							? 4
							: 6)));
