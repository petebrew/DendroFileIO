...
public class HeikkenenChronoWriter extends AbstractDendroCollectionWriter {

	private TridasToHeikkenenChronoDefaults defaults;
	private INamingConvention naming = new NumericalNamingConvention();
	
	public HeikkenenChronoWriter() {
		super(TridasToHeikkenenChronoDefaults.class, new HeikkenenChronoFormat());
	}
	...
	protected void parseTridasProject(TridasProject argProject,
		...
		defaults = (TridasToHeikkenenChronoDefaults) argDefaults;
					...
					for (TridasRadius r : s.getRadiuses()) {											
						for (TridasMeasurementSeries ms : r.getMeasurementSeries()) {					
							for (int i = 0; i < ms.getValues().size(); i++) {
								...
								HeikkenenChronoFile file = new HeikkenenChronoFile(defaults);
								
							
								// Convert units and add data to file
								try {
									file.setDataValues(UnitUtils.convertTridasValues(NormalTridasUnit.MICROMETRES, tvsgroup, false));
