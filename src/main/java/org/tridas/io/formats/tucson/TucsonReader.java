...
public class TucsonReader extends AbstractDendroFileReader {
				...
				case RWL_DATA_COMPLETE_8 :
					...
					if (isChronology) {
						if (withinChronologyBlock == false) {
							// Remove the skeleton radius entity and replace with
							// radiusPlaceHolder
	...
	private void loadRWLData(String line) throws InvalidDendroFileException {
		...
		for (String value : vals) {
			...
			if (value.equals("999")) {
				...
				addWarning(new ConversionWarning(WarningType.INVALID, "Data values contain non-numeric characters"));
				return;
			}
			else {
		...
		if ((lastSeriesCode == null) || ((lastSeriesCode != null) && (!lastSeriesCode.equals(thisCode)))) {
			...
			ArrayList<TridasValue> previouslygotvalues = (ArrayList<TridasValue>) halfdone.getValues().get(0)
					.getValues();
			...
			// If this is the last year in the block then set the interpretation
			// accordingly
			if (lastYearFlag) {
				TridasInterpretation interp = halfdone.getInterpretation();
				interp.setLastYear(currentLineYearMarker.add(thisDecadesValues.size() - 1)
	...
	private void loadCRNData(String line) throws InvalidDendroFileException {
		...
		/*
		 * String regex =
		 * "^[\\d\\w\\s]{8}[\\s\\d-]{4}([\\s\\d-]{4}\\s((\\s\\d)|(\\d\\d))){10}";
		 * Pattern p1 = Pattern.compile(regex,Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
		 * Matcher m1 = p1.matcher(line);
		 * int codeLength = 6;
		 * if(m1.find()) codeLength=8;
		...
		if ((lastSeriesCode == null) || ((lastSeriesCode != null) && (!lastSeriesCode.equals(thisCode)))) {
			...
			try {
				...
				ref.setRef(project.getObjects().get(0).getElements().get(0).getSamples().get(0).getRadiusPlaceholder()
						.getMeasurementSeriesPlaceholder());
				link.setIdRef(ref);
				linkList.add(link);
	...
	private void checkYearMarker(String thisCode, SafeIntYear currentLineYearMarker) throws InvalidDendroFileException {
		...
		if (lastYearMarker == null || !lastSeriesCode.equals(thisCode)) {
			...
			else if ((currentLineYearMarker.compareTo(lastYearMarker) < 0) && (lastSeriesCode.equals(thisCode))) {
				// Marker is less than the previous marker.
				throw new InvalidDendroFileException(I18n.getText("tucson.newSeriesSameCode", currentLineYearMarker
						.toString()), getCurrentLineNumber());
			}
			else {
				// This year marker is not within a decade of the last year marker
				throw new InvalidDendroFileException(I18n.getText("fileio.invalidDecadeMarker", currentLineYearMarker
	...
	protected void loadMetadata(String line1, String line2, String line3) {
		...
		if (isLikelyHeader(line1, line2, line3)) {
			...
			if (line2.length() > 75) {
				...
				tucsonFields.getIntegerDefaultValue(TucsonField.ELEVATION).setValue(
				...
				// tucsonFields.getStringDefaultValue(TucsonField.RANGE).setValue((line2.substring(67,
				// 75)).trim());
				log.debug("State country='"
						+ tucsonFields.getStringDefaultValue(TucsonField.STATE_COUNTRY).getStringValue() + "'");
				log.debug("Species name='"
						+ tucsonFields.getStringDefaultValue(TucsonField.SPECIES_NAME).getStringValue() + "'");
			...
			if (line3.length() > 79) {
				...
				log.debug("Investigator='"
						+ tucsonFields.getStringDefaultValue(TucsonField.INVESTIGATOR).getStringValue() + "'");
	...
	private enum TucsonLineType {
		...
		HEADER_LINE1, HEADER_LINE2, HEADER_LINE3, NON_DATA, // Any non-data line, likely
															// metdata or comment
		DATA, // Contains some sort of ring width measurements
		RWL_DATA, RWL_DATA_6, // Line with standard measurement series data (not
								// chronology)
		RWL_DATA_PARTIAL_6, // Standard measurement series data but not a full decadal
							// block
		RWL_DATA_COMPLETE_6, // Complete decadal block of measurements
		RWL_DATA_8, RWL_DATA_PARTIAL_8, // Standard measurement series data but not a full
										// decadal block
		RWL_DATA_COMPLETE_8, // Complete decadal block of measurements
		CRN_DATA, // Line with some chronology data in it
		CRN_DATA_6, CRN_DATA_PARTIAL_6, // Line with chronology data in it but not a full
										// decadal block
		CRN_DATA_COMPLETE_6, // Complete decadal block of chronology values
		CRN_DATA_8, CRN_DATA_PARTIAL_8, CRN_DATA_COMPLETE_8;
